...
L.BingLayer = L.TileLayer.extend({
  ...
  _makeApiUrl: function (restApi, resourcePath, query) {
    ...
    return (
      L.Util.template(template, baseAPIparams) + L.Util.getParamString(query)
    );
  },
  ...
  loadMetadata: function () {
    ...
    this.callRestService(request, function (meta) {
      ...
      if (r.imageUrlSubdomains) {
        options.subdomains = r.imageUrlSubdomains;
      }
      this._providers = r.imageryProviders
  ...
  _update_attribution: function (remove) {
    ...
    var attributions = this._providers.map(function (provider) {
      return remove
        ? false
        : provider.coverageAreas.some(function (area) {
            return (
              zoom <= area.zoomMax &&
              zoom >= area.zoomMin &&
